...
@patch("app.routes.workflows.get_db_pool")
@async_test
async def test_get_workflow_status_success(mock_get_db_pool):
    # Setup mock db pool
    mock_conn = AsyncMock()
    mock_pool = MagicMock()
    mock_pool.acquire.return_value.__aenter__.return_value = mock_conn
    ...
    mock_conn.fetchrow.return_value = {
        ...
        "task": "Test task",
        "history": [{"agent": "syncer_onboard", "status": "COMPLETED_AND_CLEARED"}]
    }
...
@patch("app.routes.workflows.get_db_pool")
@async_test
async def test_get_workflow_status_json_history(mock_get_db_pool):
    # Setup mock db pool
    mock_conn = AsyncMock()
    mock_pool = MagicMock()
    mock_pool.acquire.return_value.__aenter__.return_value = mock_conn
    ...
    import json
    # Mocking stringified history case
    mock_conn.fetchrow.return_value = {
        "status": "COMPLETED",
        "current_agent": None,
        "task": "Test task",
        "history": json.dumps([{"agent": "priya_promptcraft", "status": "COMPLETED_AND_CLEARED"}])
    }
    ...
    assert response["history"][0]["agent"] == "priya_promptcraft"
